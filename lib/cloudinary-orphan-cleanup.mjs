...
async function validate(root, limit, logger) {
  ...
  const [ml, shopee] = await Promise.all([scanMercadoLivre(accounts, mlSkus), scanShopee(accounts, shopeeSkus)]);
  for (const asset of stage) {
    const item = evidence.get(asset.asset_id);
  ...
  for (const asset of stage.filter(item => requiredSourcesForCandidate(item).has("tiny") && item.tiny_product_id)) {
    const item = evidence.get(asset.asset_id);
