...
export default async function ActivitiesPage({ searchParams }: { searchParams?: { page?: string } }) {
  ...
  const result = await supabaseAdmin().from("marketplace_activities")
    .select("id,marketplace,order_id,description,value,status,item_count,received_at,attempt_count,next_attempt_at,processing_error", { count: "exact" })
  ...
  return <main className="shell"><Sidebar /><section className="main">
    ...
    <section className="card"><div className="muted">Registro {count ? from + 1 : 0}-{Math.min(from + PAGE_SIZE, count)} de {count.toLocaleString("pt-BR")}</div>
      <div className="table-wrap"><table><thead><tr><th>Marketplace</th><th>ID Venda</th><th>Descricao</th><th>Valor</th><th>Fila</th><th>Tentativas</th><th>Itens</th><th>Data/Hora</th></tr></thead><tbody>
        {rows.map(row => <tr key={row.id}><td>{row.marketplace === "shopee" ? "Shopee" : "ML"}</td><td><Link href={`/atividades-marketplace/${row.id}`}>{row.order_id || "-"}</Link></td><td>{row.description || row.processing_error || "-"}</td><td>{Number(row.value || 0).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</td><td>{queueStatusLabel(row.status)}</td><td>{row.attempt_count || 0}</td><td>{row.item_count}</td><td>{new Date(row.received_at).toLocaleString("pt-BR")}</td></tr>)}
        {!rows.length && <tr><td colSpan={8}>Nenhuma atividade registrada.</td></tr>}
      </tbody></table></div>
