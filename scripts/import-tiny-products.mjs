...
for(const p of catalog){const source=`TINY_${p.id}`,linked=byTiny.get(String(p.id))||bySource.get(source)||(p.codigo?bySku.get(String(p.codigo).trim().toLowerCase()):null),sku=linked?.sku||skuOf(p),title=String(p.nome||`Produto Tiny ${p.id}`).trim();const payload={sku,source_key:source,type_code:typeOf(p),brand_code:brandOf(p),title,price:Number(p.preco||p.preco_promocional||0),stock:0,status:p.situacao==="I"?"draft":"sent",tiny_product_id:String(p.id),sent_target:"TINY",sent_at:new Date().toISOString(),updated_at:new Date().toISOString()};linked?updates.push({id:linked.id,payload}):inserts.push(payload)}
