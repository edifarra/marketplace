...
export default async function InventoryHistoryPage({ searchParams }: { searchParams?: { busca?: string; produto?: string } }) {
  ...
  let products: Array<{ id: string; sku: string; title: string; estoque: Array<{ estoque_fisico: number; estoque_disponivel: number }> }> = [];
  if (productId) {
    const result = await db.from("products").select("id,sku,title,estoque(estoque_fisico,estoque_disponivel)").eq("id", productId).limit(1).throwOnError();
    products = result.data || [];
  } else if (search) {
    const escaped = search.replace(/[%_,()]/g, "");
    const result = await db.from("products").select("id,sku,title,estoque(estoque_fisico,estoque_disponivel)")
  ...
  const movements = selected ? await db.from("estoque_movimentacao").select("id,tipo,descricao,quantidade,estoque_fisico_anterior,estoque_fisico_atual,estoque_disponivel_anterior,estoque_disponivel_atual,created_at,metadata")
    .eq("product_id", selected.id).order("created_at", { ascending: false }).order("id", { ascending: false }).throwOnError() : { data: [] };
  const stock = selected?.estoque?.[0];
  ...
  return <main className="shell"><Sidebar /><section className="main">
    ...
    {selected && <>
      ...
      <section className="section card"><h2>Histórico de Estoque</h2><div className="table-wrap"><table><thead><tr><th>Data do evento</th><th>Descrição</th><th>Movimento</th><th>Estoque físico</th><th>Estoque disponível</th></tr></thead><tbody>
        {(movements.data || []).length === 0 ? <tr><td colSpan={5}>Ainda não há movimentações registradas.</td></tr> : (movements.data || []).map(row => <tr key={row.id}><td>{formatDate(row.created_at)}</td><td>{row.descricao}<OrderInfo metadata={row.metadata} /></td><td>{Number(row.quantidade) > 0 ? "+" : ""}{row.quantidade}</td><td>{row.estoque_fisico_anterior} → <strong>{row.estoque_fisico_atual}</strong></td><td>{row.estoque_disponivel_anterior} → <strong>{row.estoque_disponivel_atual}</strong></td></tr>)}
