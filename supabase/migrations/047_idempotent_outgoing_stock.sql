-- Uma mesma versao de estoque pode ser enviada uma unica vez para cada anuncio.
-- A chave passa a ser gravada somente nas novas atividades, preservando todo o
-- historico anterior, inclusive as duplicidades que motivaram esta correcao.
alter table outgoing_marketplace_activities
  add column if not exists deduplication_key text;

create unique index if not exists idx_outgoing_stock_deduplication
  on outgoing_marketplace_activities(deduplication_key)
  where deduplication_key is not null;

-- Retorna apenas produtos cujo saldo foi efetivamente alterado. Notificacoes
-- repetidas do mesmo pedido continuam sendo processadas/auditadas, mas nao
-- voltam a distribuir uma versao de estoque que ja foi enviada.
...
begin
  ...
  loop
    changed := false;
    select e.estoque_fisico, e.estoque_disponivel into before_f, before_a
    ...
    if p_release and current_sale.inventory_reserved and not current_sale.physical_stock_deducted then
      ...
      changed := row(before_f, before_a) is distinct from row(after_f, after_a);
      ...
      after_a := least(after_f, after_a);
      update estoque e set estoque_fisico = after_f, estoque_disponivel = after_a, updated_at = now() where e.product_id = row_item.product_id;
      insert into estoque_movimentacao(product_id,sku,venda_id,tipo,descricao,quantidade,estoque_fisico_anterior,estoque_fisico_atual,estoque_disponivel_anterior,estoque_disponivel_atual,metadata)
      values(row_item.product_id,row_item.sku,p_sale_id,'VENDA_REALIZADA','Venda Realizada',-row_item.quantidade,before_f,after_f,before_a,after_a,jsonb_build_object('marketplace',current_sale.marketplace,'order_id',current_sale.order_id)) on conflict do nothing;
      changed := row(before_f, before_a) is distinct from row(after_f, after_a);
      ...
      changed := before_a is distinct from after_a;
    end if;
